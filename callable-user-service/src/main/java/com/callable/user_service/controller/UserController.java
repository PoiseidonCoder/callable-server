...
public class UserController {
    ...
    public void register(@RequestBody RegisterRequestDto registerRequestDto) {
        userService.registerService(registerRequestDto);
    }

    @PostMapping("/login")
    public ResponseEntity<?> login(@RequestBody LoginRequestDto loginRequestDto) {
        LoginResponseDto loginReponseDto = userService.loginService(loginRequestDto);
        return ResponseEntity.ok(loginReponseDto);
    }

    @PostMapping("/google")
    public ResponseEntity<LoginResponseDto> loginGoogle(@RequestBody LoginGoogleRequestDto dto) {
